...
describe('start_town_targeting', () => {
  it('parks the spell and switches mode without spending anything', () => {
    const s = inTown();
    const pc = caster(s);
    const sp = pc.curSp;
  ...
  it('cancelling is free and puts the mode back', () => {
    const s = inTown();
    const pc = caster(s);
    const sp = pc.curSp;
    startTownTargeting(s, Spell.QUICKFIRE, 0);
  ...
  it('substitutes a single square for the one rotatable pattern', () => {
    const s = inTown();
    caster(s);
...
describe('cast_town_spell', () => {
  it('Quickfire lights the square, and the cost is paid now', () => {
    const s = inTown();
    const pc = caster(s);
    const sp = pc.curSp;
    const at = nearby(s);
  ...
  it('refuses a square outside the town, without charging', () => {
    const s = inTown();
    const pc = caster(s);
    const sp = pc.curSp;
    startTownTargeting(s, Spell.QUICKFIRE, 0);
    castTownSpell(s, { x: 0, y: 0 });
    expect(s.univ.transcript.at(-1)).toBe("  Can't target outside town.");
    expect(pc.curSp).toBe(sp);
    expect(s.mode).toBe(GameMode.TOWN);
  });

  it('the barriers go down on a clear square', () => {
    const s = inTown();
    caster(s);
    const at = nearby(s);
  ...
  it('a barrier is blocked by a monster standing on the square', () => {
    const s = inTown();
    caster(s);
    const at = nearby(s);
  ...
  it('Antimagic lays a cloud with its corners cut off', () => {
    const s = inTown();
    caster(s);
    const at = nearby(s);
  ...
  it('Scry Monster notes what is there, and says when nothing is', () => {
    const s = inTown();
    caster(s);
    const at = nearby(s);
    for (const m of s.univ.town!.monsters) {
  ...
  it('Unlock refuses terrain that is not a lock', () => {
    const s = inTown();
    caster(s);
    const at = nearby(s);
    const town = s.univ.town!;
  ...
  it('Dispel Barrier reports an empty square', () => {
    const s = inTown();
    caster(s);
    const at = nearby(s);
  ...
  it('Dispel Barrier breaks a force cage', () => {
    const s = inTown();
    caster(s);
    const at = nearby(s);
  ...
  it('Dispel Barrier eventually breaks a real barrier', () => {
    const s = inTown();
    caster(s);
    const at = nearby(s);
    const town = s.univ.town!;
  ...
  it('a click with nothing in the air does nothing at all', () => {
    const s = inTown();
    const pc = caster(s);
    const sp = pc.curSp;
    const before = s.univ.transcript.length;
    castTownSpell(s, nearby(s));
...
describe('the whole flow, from do_mage_spell', () => {
  it('Quickfire cast in town targets, then lands where it is pointed', () => {
    const s = inTown();
    const pc = caster(s);
    pc.traits[Trait.PACIFIST] = false;
