...
describe('hit_space', () => {
  it('an antimagic field swallows fire, cold and magic but not weapon damage', async () => {
    const s = inTown();
    ...
    await hitSpace(s, at, 20, DamageType.FIRE, 1, 1, WHO_HIT_PARTY);
    await hitSpace(s, at, 20, DamageType.COLD, 1, 1, WHO_HIT_PARTY);
    await hitSpace(s, at, 20, DamageType.MAGIC, 1, 1, WHO_HIT_PARTY);
    expect(pc.curHealth).toBe(before);
    await hitSpace(s, at, 20, DamageType.WEAPON, 1, 1, WHO_HIT_PARTY);
    expect(pc.curHealth).toBeLessThan(before);
  });

  it('says so rather than hitting when the damage is nothing', async () => {
    const s = inTown();
    const at = partyAlone(s);
    const pc = s.univ.party.pcs[0]!;
    const before = pc.curHealth;
    await hitSpace(s, at, 0, DamageType.FIRE, 1, 1, WHO_HIT_PARTY);
    expect(s.univ.transcript.at(-1)).toBe('  No damage.');
    expect(pc.curHealth).toBe(before);
  });

  it('hit_pcs_in_space leaves the monsters out of it', async () => {
    const s = inTown();
    const at = partyAlone(s);
    ...
    await hitPcsInSpace(s, at, 15, DamageType.FIRE, 1, 1);
    expect(monst.health).toBe(monstBefore);
    expect(s.univ.party.pcs[0]!.curHealth).toBeLessThan(pcBefore);
  });

  it('out of combat one blow lands on the whole party', async () => {
    const s = inTown();
    const at = partyAlone(s);
    const before = s.univ.party.pcs.map((p) => p.curHealth);
    await hitPcsInSpace(s, at, 12, DamageType.FIRE, 1, 1);
...
describe('monst_inflict_fields', () => {
  it('quickfire burns a monster whether or not it radiates anything', async () => {
    const s = inTown();
    const monst = aLiveMonster(s);
  ...
  it('a monster that radiates nothing walks through a wall of blades unhurt', async () => {
    const s = inTown();
    const monst = aLiveMonster(s);
  ...
  it('a web catches a monster and is used up doing it', async () => {
    const s = inTown();
    const monst = aLiveMonster(s);
  ...
  it('a bug walks straight through a web', async () => {
    const s = inTown();
    const monst = aLiveMonster(s);
  ...
  it('only the first field on a square bites — the C++ breaks out', async () => {
    const s = inTown();
    const monst = aLiveMonster(s);
    if (monst.mon.race === Race.BUG) monst.mon.race = Race.HUMANOID;
    const { x, y } = monst.curLoc;
  ...
  it('flattens a crate it is standing on and spills what it held', async () => {
    const s = inTown();
    const monst = aLiveMonster(s);
    const { x, y } = monst.curLoc;
...
describe('force cages', () => {
  it('sync puts a barrier under anyone who still has the status', async () => {
    const s = inTown();
    const pc = s.univ.party.pcs[0]!;
  ...
  it('sync catches someone who walked onto a barrier without the status', async () => {
    const s = inTown();
    const pc = s.univ.party.pcs[0]!;
    const at = pc.getLoc();
  ...
  it('a cage that ticks to zero flickers out and lets go', async () => {
    const s = inTown();
    const pc = s.univ.party.pcs[0]!;
    const at = pc.getLoc();
    s.univ.town!.setField(at.x, at.y, FieldType.BARRIER_CAGE, true);
  ...
  it('does nothing where there is no cage', async () => {
    const s = inTown();
    const pc = s.univ.party.pcs[0]!;
    pc.status[Status.FORCECAGE] = 5;
...
describe('process_fields', () => {
  ...
  it('fields eventually burn out on their own', async () => {
    const s = inTown();
    const at = partyAlone(s);
  ...
  it('quickfire spreads to its neighbours', async () => {
    const s = inTown();
    const town = s.univ.town!;
  ...
  it('does nothing at all outdoors', async () => {
    const s = inTown();
    const before = s.univ.party.pcs.map((p) => p.curHealth);
    s.univ.town = null;
    await processFields(s);
  ...
  it('a town with no quickfire never runs the spread pass', async () => {
    const s = inTown();
